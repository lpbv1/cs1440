...
public:
    ...
    bool Found(T newKey);
    KeyValue<T, S>& update(T Key);
    void add(T newKey, S newValue);
    int find(const T& key);
    void removeByIndex(int index);
    void removeByKey(const T& Key);
    KeyValue<T, S>& getByIndex(int index);
    KeyValue<T, S>& operator[](int index);
    KeyValue<T, S>& getByKey(const T& key);
...
template<typename T, typename S>
KeyValue<T, S>& Dictionary<T, S>::operator[](int index) {
...
template <typename T, typename S>
KeyValue<T, S>& Dictionary<T, S>::update(T Key){
    return getByKey(Key);
}
...
template <typename T, typename S>
 KeyValue<T, S>& Dictionary<T, S>::getByKey(const T& key){
    int index = find(key);
    return getByIndex(index);
}

template<typename T, typename S>
 KeyValue<T, S>& Dictionary<T, S>::getByIndex(int index) {
